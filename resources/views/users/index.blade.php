        ...
        <table id="usersTable" class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase">NPM</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @foreach ($users as $user)
                    <tr>
                        ...
                        <td class="px-6 py-4 text-gray-700">{{ $user->npm ?? 'staff' }}</td>
            ...
            <h2 class="text-lg font-semibold mb-4">Tambah Pengguna</h2>
                ...
                <div>
                    ...
                    <div>
                        ...
                        <input type="text" name="npm" value="{{ old('npm') }}" class="w-full border border-gray-300 rounded-md px-3 py-2 mt-1 text-sm">
                        @error('npm')
                            <div class="text-red-500 text-xs mt-2">{{ $message }}</div>
                        @enderror
                    </div>
    ...
    <div class="min-h-screen flex items-center justify-center py-6 px-4">
        <div class="bg-white w-full max-w-md mx-auto rounded-lg shadow-lg p-6 relative">
            ...
            action="{{ route('users.update', ['user' => '__ID__']) }}">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700">NPM</label>
                    <input type="text" name="npm" id="editNpm" value="{{ old('npm') }}" class="w-full border border-gray-300 rounded-md px-3 py-2 mt-1 text-sm">
                    @error('npm')
                        <div class="text-red-500 text-xs mt-2">{{ $message }}</div>
                    @enderror
                </div>
...
<script>
    function openEditModal(user) {
        ...
        document.getElementById('editNpm').value = user.npm ?? '';
